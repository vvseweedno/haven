...
export type DashboardMode = "home" | "observatory";

export function Dashboard({ mode = "home" }: { mode?: DashboardMode }) {
      ...
      <div className="dashboard page-shell dashboard-home">
        ...
        <section className="decision-overview" aria-labelledby="decision-overview-title">
          <div className="decision-overview-heading">
            <div>
              <p className="eyebrow">
                {locale === "ru" ? "Один путь вместо лабиринта разделов" : "One path instead of a maze of features"}
              </p>
            ...
            <Link prefetch={false} href="/observatory" className="text-link">
        ...
        <section className="decision-boundary-summary">
          <div>
            <p className="eyebrow">{locale === "ru" ? "Что можно проверить сейчас" : "What you can verify now"}</p>
            <h2>{locale === "ru" ? "Рабочие локальные доказательства" : "Working local evidence"}</h2>
            <p>
              {locale === "ru"
          ...
          <div>
            <p className="eyebrow">{locale === "ru" ? "Что остаётся архитектурой" : "What remains architecture"}</p>
            <h2>{locale === "ru" ? "Не путать с работающим сервисом" : "Do not mistake design for a live service"}</h2>
            <p>
              {locale === "ru"
